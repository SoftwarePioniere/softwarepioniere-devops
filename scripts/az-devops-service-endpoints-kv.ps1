...
foreach ($ep in $endpoints) {
  ...
  $principalId = (az keyvault secret show --vault-name $keyVaultName --name $key --subscription $keyVaultSubscription --query 'value' --output tsv)
  if ($LASTEXITCODE -eq 0) {
    
    $key = "$($princ)-principal-secret"
    $principalSecret = (az keyvault secret show --vault-name $keyVaultName --name $key --subscription $keyVaultSubscription --query 'value' --output tsv)
    if ($LASTEXITCODE -ne 0) { throw 'error' }     
  
    $key = "$($princ)-subscription-id"
    $subscriptionId = (az keyvault secret show --vault-name $keyVaultName --name $key --subscription $keyVaultSubscription --query 'value' --output tsv)
    if ($LASTEXITCODE -ne 0) { throw 'error' }     
  
    $key = "$($princ)-subscription-name"
    $subscriptionName = (az keyvault secret show --vault-name $keyVaultName --name $key --subscription $keyVaultSubscription --query 'value' --output tsv)
    if ($LASTEXITCODE -ne 0) { throw 'error' }     
  
    $key = "$($princ)-tenant-id"
    $tenantId = (az keyvault secret show --vault-name $keyVaultName --name $key --subscription $keyVaultSubscription --query 'value' --output tsv)
    if ($LASTEXITCODE -ne 0) { throw 'error' }     
